...
/***/ function(module, exports, __webpack_require__) {
	...
	// 	menuOption: []
	// });
	window.Panel = window.Panel || _Panel2.default;
	// alert(1)
	...
	var Panel = function () {
		function Panel(_ref) {
			...
			var container = _ref.container,
			    ...
			    _ref$menuOption = _ref.menuOption,
			    menuOption = _ref$menuOption === undefined ? [] : _ref$menuOption,
			    _ref$shapeOption = _ref.shapeOption,
			    shapeOption = _ref$shapeOption === undefined ? {} : _ref$shapeOption;
			...
			this.resizeFn = function () {
				...
				var widthRes = regx.exec(width),
				    heightRes = regx.exec(height);
				if (widthRes) {
					_this.width = widthRes.pop() / 100 * _this.containerParent.offsetWidth;
				} else {
					_this.width = parseInt(width);
				}
				if (heightRes) {
					_this.height = heightRes.pop() / 100 * _this.containerParent.offsetHeight;
				} else {
					_this.height = parseInt(height);
				}

				_this.bgCanvas.width = _this.frontCanvas.width = _this.width;
				_this.bgCanvas.height = _this.frontCanvas.height = _this.height;
				_this.initBackground();
				_this.offset = _this.countOffset(_this.frontCanvas);
				_this.repaint();
			};
			if (widthRes) {
			...
			var renameKeyboardInputTimer = void 0;
			...
			var changeText = function changeText(text) {
				if (_this.renamingShape) {
					var elementHeight = _this.renameInput.clientHeight;
					_this.renamingShape.setText(text);
					if (elementHeight < _this.renamingShape.initial.height) {
						// 小于默认高度则重置到默认值
						_this.renamingShape.resetDimension();
						_this.renamingShape.setDimension();
					} else {
						_this.renamingShape.setDimension({ height: elementHeight });
					}
					// this.renamingShape.isShow = true;
					_this.repaint();
				}
			};
			this.renameInput.onmouseup = this.renameInput.onkeyup = this.renameInput.onkeydown = function () {
				// this.renamingShape.isShow = false;
				// this.repaint();
				renameKeyboardInputTimer = setTimeout(changeText.bind(_this, _this.renameInput.innerHTML), 0);
			};
			this.renameInput.onpaste = function (e) {
				clearTimeout(renameKeyboardInputTimer);
				setTimeout(function () {
					e.target.innerHTML = e.target.textContent;
					changeText(_this.renameInput.innerHTML);
					_this.setCursorAtEnd(_this.renameInput);
				}, 0);
			};
			this.renameInput.onblur = function () {
				clearTimeout(renameKeyboardInputTimer);
				_this.renameInput.style.zIndex = -1;
				_this.renameInput.style.visibility = 'hidden';
				_this.renamingShape.isShow = true;
				_this.repaint();
				_this.renamingShape = null;
			};
			...
			this.shapeRule['Line'] = _Shape.Line;
		...
		_createClass(Panel, [{
			key: 'findActiveLine',
			value: function findActiveLine(mouseX, mouseY, callback) {

				var pointInCanvasLeft = mouseX - this.offset.left,
			...
			key: 'setCursorAtEnd',
			value: function setCursorAtEnd(DOMElement) {
				// 设置光标
			...
			value: function initEvents() {
				...
				this.frontCanvas.oncontextmenu = function (e) {
					...
					var startX = e.pageX,
					    startY = e.pageY;
					_this2.findActiveShape(startX, startY);
					_this2.menu.show({ startX: startX, startY: startY });
					e.stopPropagation();
				};
				var onmousemove = void 0,
				    line = {};
				this.frontCanvas.addEventListener('mousedown', function (e) {
					_this2.offset = _this2.countOffset(_this2.frontCanvas);
					// 避免鼠标超出边界后回来，事件没有注销
					onmousemove && _this2.frontCanvas.removeEventListener('mousemove', onmousemove);
					var startX = e.pageX,
					    startY = e.pageY;
					_this2.menu.hide();
					// 由于层级覆盖，先检查上层的图形
					if (e.button === 0) {
						// 鼠标左键
						_this2.findActiveShape(startX, startY, function (activedShape, activedDot) {
							if (!activedShape) {
								// 全局拖动
								onmousemove = function onmousemove(e) {
									_this2.move = true;
									_this2.repaint(e.pageX - startX, e.pageY - startY);

									_this2.event.emit('mousemove', e);
								};
							} else {
								onmousemove = function onmousemove(e) {

									if (_this2.drawLine && line.path) {
										// 连线的末端点移动
										line.path.close({ x: e.pageX - _this2.offset.left, y: e.pageY - _this2.offset.top });
									} else {
										_this2.activedShape && _this2.activedShape.setPosition(e.pageX - startX, e.pageY - startY);
									}
									_this2.repaint();
									if (_this2.drawLine) {
										// 仅在连线的时候检测鼠标移动过程中碰到的图形和点，防止因鼠标移动导致activeShape被替换
										_this2.findActiveShape(e.pageX, e.pageY, function (activedShape, activedDot) {
											// 在连线的过程中碰到了图形，则显示连接点
											if (activedShape) {
							...
							if (_this2.drawLine && activedDot) {
								...
								line = _this2.addLine({ x: e.pageX - _this2.offset.left, y: e.pageY - _this2.offset.top });
							}
							_this2.frontCanvas.addEventListener('mousemove', onmousemove);
						});
					}
					if (!_this2.drawLine && !_this2.activedShape) {
						_this2.findActiveLine(startX, startY, function (activedLine) {
					...
					_this2.event.emit('mousedown', e);
				});
				this.frontCanvas.addEventListener('dblclick', function (e) {
					if (_this2.activedShape) {
						_this2.activedShape.isShow = false;
						_this2.repaint();
						// 先会出发两次mousedown，然后触发dblclick
						_this2.renamingShape = _this2.activedShape;

						var _renamingShape$export = _this2.renamingShape.exportMetaData(),
						    _renamingShape$export2 = _renamingShape$export.position,
						    x = _renamingShape$export2.x,
						    y = _renamingShape$export2.y,
						    width = _renamingShape$export.width,
						    height = _renamingShape$export.height,
						    data = _renamingShape$export.data,
						    size = _renamingShape$export.font.size;

						_this2.renameInput.innerHTML = data.text;
						_this2.renameInput.style.cssText = 'top:' + y + 'px; left:' + x + 'px; width:' + width + 'px; min-height:' + _this2.renamingShape.initial.height + 'px; z-index:999999; font-size:' + size + 'px; visibility:visible;';
						// 设置光标位置
						_this2.setCursorAtEnd(_this2.renameInput);
					}
					_this2.event.emit('dblclick', e);
				});
				this.frontCanvas.addEventListener('mouseup', function (e) {
					_this2.frontCanvas.removeEventListener('mousemove', onmousemove);
					_this2.activedShape && _this2.activedShape.drop();

					// 处于连线状态
					_this2.drawLine = false;
					// 处于移动图形状态
					if (_this2.move) {
						...
						try {
							for (var _iterator3 = _this2.shapes[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
								var s = _step3.value;
					...
					if (line.path && (line.endDot < 0 || line.endShapeId == line.startShapeId)) {
						_this2.deleteLine(line);
					}
					line = {};
					_this2.frontCanvas.style.cursor = 'default';
					var startX = e.pageX,
					    startY = e.pageY;
					_this2.findActiveShape(startX, startY, function (activedShape) {
						if (activedShape) {
					...
					_this2.event.emit('mouseup', e);
				});
				this.frontCanvas.addEventListener('drop', function (e) {
					e.preventDefault();
					_this2.offset = _this2.countOffset(_this2.frontCanvas);

					var dataString = e.dataTransfer.getData('text');
					try {
						...
						_this2.addShape('Rectangle', {
							x: e.pageX - _this2.offset.left,
							y: e.pageY - _this2.offset.top,
							data: data,
							canvasContext: _this2.frontCtx,
							color: _this2.shapeOption.color,
							backgroundColor: _this2.shapeOption.backgroundColor,
							font: _this2.shapeOption.font
						});
						_this2.event.emit('drop', e);
					} catch (e) {
				...
				try {
					for (var _iterator4 = this.paths[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
						...
						if (r.startShapeId && r.endShapeId) {
							...
							try {
								for (var _iterator6 = this.shapes[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
									var s = _step6.value;
							...
							if (sdot && edot && sdot != edot) {
								var pathStart = { x: sdot.position.x, y: sdot.position.y },
			...
			value: function deleteShape(shape) {
				...
				for (var i = 0; i < this.shapes.length; i++) {
					if (shape.id === this.shapes[i].id) {
						...
						for (var j = this.paths.length - 1; j > -1; j--) {
			...
			value: function deleteLine(line) {
				var needRepaint = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
			...
			key: 'exportCanvasData',
			value: function exportCanvasData() {
				...
				if (this.hasShape()) {
					...
					try {
						for (var _iterator7 = this.shapes[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
							var s = _step7.value;
			...
			value: function importCanvasData(metaData) {
				...
				try {
					for (var _iterator9 = metaData.shapes[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
						var s = _step9.value;
				...
				try {
					for (var _iterator10 = metaData.lines[Symbol.iterator](), _step10; !(_iteratorNormalCompletion10 = (_step10 = _iterator10.next()).done); _iteratorNormalCompletion10 = true) {
						var l = _step10.value;
						...
						var line = Reflect.construct(this.shapeRule[l.shape], [l]);
						this.paths.push(line);
					}
				} catch (err) {
			...
			value: function saveAsImage() {
				...
				try {
					for (var _iterator12 = this.shapes[Symbol.iterator](), _step12; !(_iteratorNormalCompletion12 = (_step12 = _iterator12.next()).done); _iteratorNormalCompletion12 = true) {
						var s = _step12.value;
...
/***/ function(module, exports) {
	...
	var Shape = function () {
		function Shape(_ref) {
			var id = _ref.id,
			    ...
			    _ref$font = _ref.font,
			    font = _ref$font === undefined ? {} : _ref$font;
		...
		_createClass(Shape, [{
			...
			value: function setFont() {
				var _ref2 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.font,
				    size = _ref2.size,
				    family = _ref2.family;
			...
			key: 'foldLongText',
			value: function foldLongText(longText, maxWidth, ctx) {
				// 处理富文本，转为普通文本
	...
	var Rectangle = exports.Rectangle = function (_Shape) {
		...
		function Rectangle(_ref3) {
			var id = _ref3.id,
			    _ref3$x = _ref3.x,
			    x = _ref3$x === undefined ? 10 : _ref3$x,
			    _ref3$y = _ref3.y,
			    y = _ref3$y === undefined ? 10 : _ref3$y,
			    _ref3$width = _ref3.width,
			    width = _ref3$width === undefined ? 80 : _ref3$width,
			    _ref3$height = _ref3.height,
			    height = _ref3$height === undefined ? 40 : _ref3$height,
			    data = _ref3.data,
			...
			var _this = _possibleConstructorReturn(this, (Rectangle.__proto__ || Object.getPrototypeOf(Rectangle)).call(this, { id: id, color: color, backgroundColor: backgroundColor, borderColor: borderColor, font: font }));
			// console.log(x,y)


			_this.shape = 'Rectangle';
			// 设置默认宽高
			_this.initial = { height: height, width: width };
		...
		_createClass(Rectangle, [{
			...
			value: function setDimension() {
				var _ref4 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.initial,
			...
			value: function setText() {
				var text = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.text;
			...
			value: function setContext(ctx) {
				this.ctx = ctx;
				this.dots = [new Dot({ x: this.position.x + this.width / 2, y: this.position.y, canvasContext: this.ctx, borderColor: '#ff0000' }), // 上
			...
			value: function draw() {
				...
				// 处理长文本换行
				var textArray = this.foldLongText(this.text, this.width, ctx);
				if (textArray.length === 1) {
					ctx.textBaseline = 'middle';
					ctx.fillText(textArray[0], this.position.x + this.width / 2, this.position.y + this.height / 2, this.width);
				} else {
					textArray.forEach(function (o, i) {
						ctx.fillText(o, _this2.position.x + _this2.width / 2, _this2.position.y + _this2.font.size * 1.05 * i, _this2.width);
					});
				}
				ctx.closePath();
			}
			...
			key: 'findDot',
			value: function findDot(dot) {
				return this.dots.indexOf(dot);
			}
		}, {
			key: 'findDotByIndex',
			value: function findDotByIndex(index) {
			...
			key: 'exportMetaData',
			value: function exportMetaData() {
				var superMetaData = _get(Rectangle.prototype.__proto__ || Object.getPrototypeOf(Rectangle.prototype), 'exportMetaData', this).call(this);
	...
	var Dot = function (_Shape2) {
		...
		function Dot(_ref5) {
			var id = _ref5.id,
			    ...
			    width = _ref5$width === undefined ? 8 : _ref5$width,
			    _ref5$height = _ref5.height,
			    height = _ref5$height === undefined ? 8 : _ref5$height,
			    canvasContext = _ref5.canvasContext,
			    color = _ref5.color,
			...
			var _this3 = _possibleConstructorReturn(this, (Dot.__proto__ || Object.getPrototypeOf(Dot)).call(this, { color: color, backgroundColor: backgroundColor, borderColor: borderColor, font: font }));

			_this3.shape = 'Dot';
			// this.position = {x: x - width/2, y: y - height/2};
			_this3.width = width;
			_this3.height = height;
			_this3.ctx = canvasContext;
			_this3.center = { x: x, y: y };
			_this3.setPosition({ x: x, y: y });
			return _this3;
		}

		_createClass(Dot, [{
			key: 'setPosition',
			value: function setPosition(_ref6) {
				var x = _ref6.x,
				    y = _ref6.y;
	...
	var Line = exports.Line = function (_Shape3) {
		_inherits(Line, _Shape3);

		function Line(_ref7) {
			var id = _ref7.id,
			    canvasContext = _ref7.canvasContext,
			    startShapeId = _ref7.startShapeId,
			    endShapeId = _ref7.endShapeId,
			    startDot = _ref7.startDot,
			    endDot = _ref7.endDot,
			    pathParams = _ref7.pathParams,
			    font = _ref7.font;

			_classCallCheck(this, Line);

			var _this4 = _possibleConstructorReturn(this, (Line.__proto__ || Object.getPrototypeOf(Line)).call(this, { id: id, font: font }));

			_this4.shape = 'Line';
			pathParams.unshift(canvasContext);
			_this4.path = Reflect.construct(Path, pathParams);
			_this4.startShapeId = startShapeId;
			_this4.endShapeId = endShapeId;
			_this4.startDot = startDot;
			_this4.endDot = endDot;
			return _this4;
		}
		...
		_createClass(Line, [{
			key: 'exportMetaData',
			value: function exportMetaData() {
				var superMetaData = _get(Line.prototype.__proto__ || Object.getPrototypeOf(Line.prototype), 'exportMetaData', this).call(this);
	...
	var Path = function () {
		...
		_createClass(Path, [{
			key: 'setColor',
			value: function setColor() {
				var color = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.defaultColor;

				this.color = color;
			}
		}, {
			key: 'begin',
			value: function begin(_ref8) {
				var x = _ref8.x,
				    y = _ref8.y;
			...
			key: 'isPointAroundPath',
			value: function isPointAroundPath(pointInCanvasLeft, pointInCanvasTop) {
				var sx = this.lines[0].x,
				    sy = this.lines[0].y,
				...
				for (var t = 0; t <= 1; t += 0.01) {
					var x = Math.pow(1 - t, 2) * sx + 2 * t * (1 - t) * ex + t * t * ex;
					if (pointInCanvasLeft - 2 <= x && x <= pointInCanvasLeft + 2) {
						var y = Math.pow(1 - t, 2) * sy + 2 * t * (1 - t) * sy + t * t * ey;
...
/***/ function(module, exports) {
	...
	var Menu = function () {
		function Menu(panel) {
			...
			this.options = [{ text: '连线', cb: this.drawLine, type: 1 },
			// {text: '复制', cb: this.copyShape, type: 1},
			// {text: '粘贴', cb: this.pasteShape, type: 2},
			// {text: '剪切', cb: this.cuteShape, type: 1}, 
			{ text: '删除', cb: this.deleteShape, type: 1 }, { text: '删除', cb: this.deleteLine, type: 3 }, { text: '修改', cb: function cb(panel, activeShape) {
					// 图形修改框
					var alterModalBg = document.querySelector("#xpanelMenuAlter");
					if (alterModalBg) {
					...
					if (activeShape.data.properties) {
						var dataHTML = '';
						for (var p in activeShape.data.properties) {
				...
				}, type: 1 }, { text: '保存为图片', cb: this.saveAsImage, type: 0 }, { text: '导出到草稿', cb: function cb(panel) {
					localStorage.setItem("xpanel", JSON.stringify(panel.exportCanvasData()));
				}, type: 0 }, { text: '从草稿导入', cb: function cb(panel) {
					var data = JSON.parse(localStorage.getItem("xpanel"));
					data && panel.importCanvasData(data);
				}, type: 0 }, { text: '全部清空', cb: function cb(panel) {
					panel.reset();
				}, type: 0 }].concat(_toConsumableArray(option));
		...
		_createClass(Menu, [{
			...
			value: function show(_ref) {
				...
				try {
					for (var _iterator2 = Array.from(this.element.children[0].children)[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
						var e = _step2.value;
				...
				var offsetLeft = startX - document.body.scrollLeft + 2,
				    offsetTop = startY - document.body.scrollTop + 2,
				    tmpHeight = this.element.clientHeight,
				    tmpWidth = this.element.clientWidth;

				console.log(offsetTop, offsetLeft, typeof offsetTop === 'undefined' ? 'undefined' : _typeof(offsetTop));
				// 判断当前位置是否接触到边界
			...
			key: 'deleteLine',
			value: function deleteLine(panel, activedLine) {
			...
			key: 'alterShape',
			value: function alterShape(panel, activedShape) {}
		}, {
			key: 'saveAsImage',
			value: function saveAsImage() {
				this.hide();
